...
<div class="center">
	...
	<div class="cms-tree" data-url-tree="$Link(getsubtree)" data-url-savetreenode="$Link(savetreenode)" data-url-updatetreenodes="$Link(updatetreenodes)" data-url-addpage="{$LinkPageAdd('AddForm/?action_doAdd=1', 'ParentID=%s&amp;PageType=%s')}" data-url-editpage="$LinkPageEdit('%s')" data-url-duplicate="{$Link('duplicate/%s')}" data-url-duplicatewithchildren="{$Link('duplicatewithchildren/%s')}" data-url-listview="{$Link('?view=list')}" data-url-deletepage="{$Link('/annihilate/%s')}" data-url-publishpage="{$Link('/publish-record/%s')}" data-url-unpublishpage="{$Link('/unpublish-record/%s')}" data-hints="$SiteTreeHints.XML" data-extra-params="SecurityID=$SecurityID">
		$SiteTreeAsUL
	</div>
</div>
