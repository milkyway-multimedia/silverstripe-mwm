...
class ShortcodableController extends \Controller {
    ...
    public function ShortcodeForm(){
        $original = new \ShortcodableController();
        $original->setRequest($this->request);

        if($form = $original->ShortcodeForm()) {
                ...
                if($types->Value()) {
                    switch($types->Value()) {
                        case 'user':
                            ...
                            $form->Fields()->push(\CompositeField::create(\FieldList::create(
                                        ...
                                        )->setDescription(_t('Shortcodable.DESC-CAPTION', 'Only used for values that will resolve to links'))->setForm($form),
                                        \DropdownField::create(
                                            'nolink',
                            ...
                            break;
                        case 'site_config':
                            $shortcodes = Utilities::map_array_to_i18n(\SiteConfig::config()->valid_shortcode_fields, 'SiteConfig');
                            natsort($shortcodes);
                        ...
                        case 'current_page':
                            $form->Fields()->push(\CompositeField::create(\FieldList::create(
                                        \FormMessageField::create(
                                            'NOTE-ADVANCED',
                                            _t('Shortcodable.NOTE-ADVANCED', 'Note: This is an advanced shortcode. It is recommended to use only use the suggested codes.')
                                        )->cms()->setForm($form),
                                        \TextField::create(
                                            'field',
                                            _t('Shortcodable.FIELD', 'Field')
                                        )->setDescription(_t('Shortcodable.DESC-CurrentPageField', 'Suggested codes: Title, Subtitle, Content, Link, AbsoluteLink'))->setForm($form),
                                        \DropdownField::create(
                                            'type',
                                            _t('Shortcodable.DISPLAY_TYPE', 'Display type'),
                                            [
                        ...
                        case 'icon':
                            ...
                            if(\HTMLEditorField::config()->prepend_icon)
                                ...
                                $iconPrepend = \TextField::create('prepend', _t('Shortcodable.ICON_PREPEND', 'Prepend'))->setAttribute('placeholder', 'icon icon-');
